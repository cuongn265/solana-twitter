...
<template>
  ...
  <div class="px-8 py-4" v-else>
    <div class="flex justify-between">
      ...
      <div class="flex" v-if="isMyTweet">
        <button @click="isEditing = true"
          class="flex px-2 rounded-full text-gray-500 hover:text-pink-500 hover:bg-gray-100" title="Update tweet">
        ...
        <button @click="onDelete" class="flex px-2 rounded-full text-gray-500 hover:text-pink-500 hover:bg-gray-100"
          title="Delete tweet">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 m-auto" viewBox="0 0 20 20" fill="currentColor">
            <path fill-rule="evenodd"
    ...
    <router-link v-if="tweet.topic" :to="{ name: 'Topics', params: { topic: tweet.topic } }"
      class="inline-block mt-2 text-pink-500 hover:underline break-all">
      #{{ tweet.topic }}
    </router-link>
  </div>
</template>